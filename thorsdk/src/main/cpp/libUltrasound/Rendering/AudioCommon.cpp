...
#define STREAM_CALL(c) AAudioStream_##c((AAudioStream*)stream)
    ALOGI("StreamID: %p", stream);

    ALOGI("BufferCapacity: %d", STREAM_CALL(getBufferCapacityInFrames));
    ALOGI("BufferSize: %d", STREAM_CALL(getBufferSizeInFrames));
    ALOGI("FramesPerBurst: %d", STREAM_CALL(getFramesPerBurst));
    ALOGI("XRunCount: %d", STREAM_CALL(getXRunCount));
    ALOGI("SampleRate: %d", STREAM_CALL(getSampleRate));
    ALOGI("SamplesPerFrame: %d", STREAM_CALL(getChannelCount));
    ALOGI("DeviceId: %d", STREAM_CALL(getDeviceId));
    ALOGI("Format: %s",  FormatToString(STREAM_CALL(getFormat)));
    ALOGI("SharingMode: %s", (STREAM_CALL(getSharingMode)) == AAUDIO_SHARING_MODE_EXCLUSIVE ?
                          "EXCLUSIVE" : "SHARED");
    ...
    ALOGI("PerformanceMode: %s", perfModeDescription.c_str());

    aaudio_direction_t  dir = STREAM_CALL(getDirection);
    ALOGI("Direction: %s", (dir == AAUDIO_DIRECTION_OUTPUT ? "OUTPUT" : "INPUT"));
    if (dir == AAUDIO_DIRECTION_OUTPUT) {
        ALOGI("FramesReadByDevice: %d", (int32_t)STREAM_CALL(getFramesRead));
        ALOGI("FramesWriteByApp: %d", (int32_t)STREAM_CALL(getFramesWritten));
    } else {
        ALOGI("FramesReadByApp: %d", (int32_t)STREAM_CALL(getFramesRead));
        ALOGI("FramesWriteByDevice: %d", (int32_t)STREAM_CALL(getFramesWritten));
    }
#undef STREAM_CALL
}
