...
public:
    DopplerAcq	(const std::shared_ptr<UpsReader>& upsReader);
    DopplerAcq	() {};
    ~DopplerAcq	();
    ...
    ThorStatus calculateBeamParams(uint32_t imagingCaseId, float gateStart, uint32_t gateIndex, float gateAngle, uint32_t actualPrfIndex, bool isTDI);
...
private:
    void calculateTxConfig(uint32_t imagingCaseId, float gateStart, float gateRange, float gateAngle, bool isTDI );
    ...
    void calculateBpfLut(const uint32_t imagingCaseId, uint32_t& oscale, bool isTDI );
