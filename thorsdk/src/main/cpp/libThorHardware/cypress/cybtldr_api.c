...
{
    int err = g_comm->WriteData(g_comm->priv, inBuf, inSize);

    if (CYRET_SUCCESS == err)
        err = g_comm->ReadData(g_comm->priv, outBuf, outSize);
...
int CyBtldr_StartBootloadOperation(CyBtldr_CommunicationsData* comm, unsigned long expSiId, unsigned char expSiRev, unsigned long* blVer)
{
    ...
    err = g_comm->OpenConnection(g_comm->priv);
    if (CYRET_SUCCESS != err)
        err |= CYRET_ERR_COMM_MASK;
...
int CyBtldr_EndBootloadOperation(void)
{
    ...
    if (CYRET_SUCCESS == err)
    {
        err = g_comm->WriteData(g_comm->priv, inBuf, inSize);

        if (CYRET_SUCCESS == err)
            err = g_comm->CloseConnection(g_comm->priv);
