...
void ViewMatrix3x3::multiplyMV(float *m, float *lhs, float *rhsv) {
    float t[3];
    for (int j = 0; j < 3; j++) {
        float sum = 0.0f;
        for (int i = 0; i < 3; i++) {
...
void ViewMatrix3x3::multiplyMM(float *m, float *lhs, float *rhs) {
    float t[9];
    for (int j = 0; j < 3; j++) {
        for (int i = 0; i < 3; i++) {
            float sum = 0.0f;
...
void ViewMatrix3x3::inverseM(float* m, float* in) {
    float t[9];
