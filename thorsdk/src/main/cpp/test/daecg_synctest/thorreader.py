...
class ThorReader:
    ...
    def initVar(self):
        ...
        self.mFileVersion = -1
        self.mDataType = -1
        self.mFrameInterval = -1
        self.mDisplayFileInfo = True
        #
    ...
    def setDisplayFileInfo(self, displayInfo):
        self.mDisplayFileInfo = displayInfo
    ...
    def readFile(self):
        ...
        #
        fd.seek(frame_int_loc)
        frame_interval = np.fromfile(fd, dtype=np.int32, count=1)[0]
        self.mFrameInterval = frame_interval
        #
        fd.seek(datatype_loc)
        data_type = np.fromfile(fd, dtype=np.int32, count=1)[0]
        self.mDataType = data_type
        #
        if (self.mDisplayFileInfo):
    ...
    def readBTimeStamp(self):
        ...
        fd = open(self.mFileName, "r")
        #
        out_data = np.zeros([self.mFrameCount], dtype = np.uint64)
        #
        for i in range(self.mFrameCount):
            # frameheader + offset 4
            start_loc = self.mBLoc + self.mBmodeHeaderSize + 4 + (self.mBMaxFrameSize + self.mFrameHeaderSize) * i
            fd.seek(start_loc)
            # read
            timeStamp = np.fromfile(fd, dtype=np.uint64, count=1)
            #
            out_data[i] = timeStamp
        #
        fd.close()
        #
        return out_data

    def readECGTimeStamp(self):
        #
        #
        ecg_header_loc = self.mECGLoc
        #
        ...
        #
        for i in range(self.mFrameCount):
            # timeStamp Loc Offset: 4
            data_loc = ecg_data_loc + 4 + (ecg_max_frame_size + cineheader_size) * i
            fd.seek(data_loc)
            #read
            timeStamp = np.fromfile(fd, dtype=np.uint64, count=1)
            out_data[i] = timeStamp
    ...
    def readDATimeStamp(self):
        ...
        idx = 0;
        for i in range(self.mFrameCount):
            data_loc = da_data_loc + 4 + (da_max_frame_size + cineheader_size) * i
            fd.seek(data_loc)
            #read
            timeStamp = np.fromfile(fd, dtype=np.uint64, count=1)
            out_data[i] = timeStamp
