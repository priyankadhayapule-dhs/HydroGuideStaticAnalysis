...
object IOUtility {
    ...
    fun readAllBytes(inputStream: InputStream): ByteArray {
        return readNBytes(inputStream, Int.MAX_VALUE)
    }

    @Throws(IOException::class)
    fun readNBytes(inputStream: InputStream, len: Int): ByteArray {
        // Code copied from InputStream.java source, converted to Kotlin
        ...
        var n: Int
        do {
            var buf = ByteArray(remaining.coerceAtMost(DEFAULT_BUFFER_SIZE))
            var nread = 0
